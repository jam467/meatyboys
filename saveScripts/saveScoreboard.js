...
fs.readFile(loc+'downloads/round.json', function(err, data) {
	...
	request({
		...
		url:'https://statsapi.foxsports.com.au/3.0/api/sports/rugby/series/1/currentseason.json?userkey=A00239D3-45F6-4A0A-810C-54A347F144C2'}, async function (error, response, body) {
			...
			fs.writeFile(loc+'downloads/scoreboard'+round+'.json', JSON.stringify(resolvePlayers), function (err) {
				//if (err) throw err;
				console.log('Saved!');
			});
...
function scoreboard(matchID){
	return new Promise((resolve,reject)=>{
		...
		request({
			gzip: true,
			json:true,
			url:'https://statsapi.foxsports.com.au/3.0/api/sports/rugby/matches/'+matchID+'/scoreboard.json?userkey=A00239D3-45F6-4A0A-810C-54A347F144C2'}, function (error, response, body) {
